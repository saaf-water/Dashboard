    ...
    return (
      <div className="right-0 pl-1 flex flex-row justify-between items-center">
        ...
        <Listbox value={selected} onChange={setSelected}>
          {({ open }) => (
            <>
              <Listbox.Label className="hidden sm:block flex-grow text-xs md:text-sm text-right font-medium text-gray-700 px-2">
                Location
              </Listbox.Label>
              <div className="mt-1 w-36 sm:w-60 relative">
                <Listbox.Button className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  <span className="flex items-center justify-start">
                    <LocationMarkerIcon
                      className="text-red-600 flex-shrink-0 h-6 w-6"
                      aria-hidden="true"
                    />
                    <span className="ml-2 text-xs sm:text-sm block truncate">
                      {selected.location}
                    </span>
                  </span>
